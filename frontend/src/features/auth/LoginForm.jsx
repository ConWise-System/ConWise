  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex flex-col items-center justify-between py-12 px-4 font-sans selection:bg-[#0a1120] selection:text-white overflow-hidden">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none">
        <motion.div animate={{ scale: [1, 1.1, 1], opacity: [0.3, 0.5, 0.3] }} transition={{ duration: 10, repeat: Infinity }} className="absolute top-[-15%] right-[-5%] w-[50%] h-[50%] bg-blue-100/60 rounded-full blur-[120px]" />
        <motion.div animate={{ scale: [1, 1.05, 1], opacity: [0.2, 0.4, 0.2] }} transition={{ duration: 8, repeat: Infinity, delay: 1 }} className="absolute bottom-[-10%] left-[-5%] w-[40%] h-[40%] bg-slate-200/60 rounded-full blur-[100px]" />
      </div>

      {/* Brand Header */}
      <motion.header initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="z-10 flex flex-col items-center gap-2">
        ...
        <h1 className="text-[10px] font-black tracking-[0.5em] text-slate-400 uppercase mt-2">Horizon Executive Terminal</h1>
      </motion.header>

      {/* Main Login Card */}
      <motion.div variants={containerVars} initial="hidden" animate="visible" className="z-10 w-full max-w-[480px] bg-white/80 backdrop-blur-3xl rounded-[2.5rem] shadow-[0_30px_70px_-15px_rgba(0,0,0,0.1)] border border-white p-10 md:p-14 relative">
        ...
        <div className="text-center mb-10">
          ...
          <motion.h2 variants={itemVars} className="text-4xl font-black text-[#0a1120] tracking-tighter mb-2">Welcome Back</motion.h2>
          <motion.p variants={itemVars} className="text-slate-500 text-sm font-medium">Authorized personnel only. Initialize session.</motion.p>
        </div>
        ...
        <motion.div variants={itemVars} className="mt-10 pt-8 border-t border-slate-50 text-center">
          <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">System Security Protocol v4.0.2</p>
        </motion.div>
      </motion.div>

      <motion.footer initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="z-10 text-[9px] font-black text-slate-400 tracking-[0.4em] uppercase">
...
const InputField = ({ label, icon, placeholder, type = "text", name, value, onChange }) => (
  <div className="flex flex-col gap-2">
    ...
    <div className="relative group">
      <div className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-black transition-all">{icon}</div>
      <input 
        type={type} 
        name={name}
        value={value}
